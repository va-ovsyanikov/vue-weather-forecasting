        ...
        <div class="home" v-if="isPage === ''">
            <div class="home__top">
                ...
                <font-awesome-icon
                    ...
                    @click="getPage('settings')"
                />
            </div>
            <div class="home__content">
                <div class="home-item__loading" v-if="!data.length">
                    Loading....
                </div>
                <div
                    class="home-item"
                ...
                >
                    ...
                    <div class="home-item__сontent-temp">
                        <img
                            ...
                            alt=""
                        />
                        <span class="home-item__temp"
                            >{{ temp(item.temp) }}&deg;C</span
                        >
                    </div>
                    <div class="home-item__content">
                        <div class="content__item content__item_recede">
                            <span class="home-item__feels"
                                >Feels like:
                                {{ temp(item.feels_like) }}&deg;c</span
                            >
                            <span class="home-item__clouds">{{
        ...
        <div class="settings" v-if="isPage === 'settings'">
            ...
            <div class="settings__content">
                <draggable v-model="cities" @change="onDrag">
                    <div
                        class="setting-item settings__drag-item"
                        v-for="city in cities"
                        :key="city.id"
                    >
                        <font-awesome-icon
                            class="setting-item__drag"
                            icon="fa-solid fa-bars"
                        />
                        <span class="setting-item__city"
                            >{{ city.name }}, {{ city.country }}</span
                        >
                        <font-awesome-icon
                            v-if="city.city_user"
                            icon="fa-solid fa-trash-can"
                            class="setting-item__desabled"
                        />
                        <font-awesome-icon
                            v-else
                            icon="fa-solid fa-trash-can"
                            class="setting-item__delete"
...
const getCities = () => {
    cities.value = localStorage.getItem('cities')
        ? (JSON.parse(localStorage.getItem('cities') as string) as Array<
              Pick<ICity, 'id' | 'name' | 'country' | 'city_user'>
          >)
        : ([] as Array<Pick<ICity, 'id' | 'name' | 'country' | 'city_user'>>)
...
const getCity = async (name: string) => {
    try {
        ...
        if (!cities.value.length) {
            ...
            const index1 = cities.value.findIndex(
                el => el.id === cityUser.id && 'city_user' in el !== true
            )
            if (index1 !== -1) {
